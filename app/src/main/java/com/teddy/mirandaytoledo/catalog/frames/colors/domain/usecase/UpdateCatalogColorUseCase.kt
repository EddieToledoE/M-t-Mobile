...
class UpdateCatalogColorUseCase(
    private val repository: CatalogColorRepository
) {
    suspend operator fun invoke(id: Int, name: String, hex: String?): Result<CatalogColor, NetworkError> {
        return repository.update(id = id, name = name, hex = hex)
    }
}
