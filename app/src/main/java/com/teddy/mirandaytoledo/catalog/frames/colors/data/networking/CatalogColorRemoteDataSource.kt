package com.teddy.mirandaytoledo.catalog.frames.colors.data.networking
...
class CatalogColorRemoteDataSource(private val httpClient: HttpClient) {

    suspend fun getAll(): Result<List<CatalogColorDto>, NetworkError> {
        return safeCall {
            httpClient.get(
                urlString = constructUrl("/api/catalog/colors")
            )
        }
    }

    suspend fun create(request: CreateCatalogColorRequest): Result<CatalogColorDto, NetworkError> {
        return safeCall {
            httpClient.post(
                urlString = constructUrl("/api/catalog/colors")
            ) {
    ...
    suspend fun update(id: Int, request: UpdateCatalogColorRequest): Result<CatalogColorDto, NetworkError> {
        return safeCall {
            httpClient.put(
                urlString = constructUrl("/api/catalog/colors/$id")
            ) {
                contentType(ContentType.Application.Json)
                setBody(request)
            }
