package com.teddy.mirandaytoledo.catalog.frames.sizes.data.networking
...
class SizeRemoteDataSource(private val httpClient: HttpClient) {

    suspend fun getAll(): Result<List<SizeDto>, NetworkError> {
        return safeCall {
            httpClient.get(
                urlString = constructUrl("/api/catalog/sizes")
            )
        }
    }

    suspend fun create(request: CreateSizeRequest): Result<SizeDto, NetworkError> {
        return safeCall {
            httpClient.post(
                urlString = constructUrl("/api/catalog/sizes")
            ) {
    ...
    suspend fun update(id: Int, request: UpdateSizeRequest): Result<SizeDto, NetworkError> {
        return safeCall {
            httpClient.put(
                urlString = constructUrl("/api/catalog/sizes/$id")
            ) {
                contentType(ContentType.Application.Json)
                setBody(request)
            }
